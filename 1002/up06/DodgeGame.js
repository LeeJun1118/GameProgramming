...
function MoveMissile() {
    for (var i = 0; i < arrMissiles.length; i++) {
        ...
        if (IsColllisionWithPlayer(arrMissiles[i].x, arrMissiles[i].y)) {
            onGameOver();
        }
...
function onGameStart() {
    ...
    arrMissiles.push({x: 700, y: 0, go_x: -1, go_y: 1});
    arrMissiles.push({x: 700, y: 500, go_x: -1, go_y: -1});
    arrMissiles.push({x: 0, y: 500, go_x: 1, go_y: -1});
}
