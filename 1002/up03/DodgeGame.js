...
var tempMissile2 = {x:700 , y:0 , go_x:-1 , go_y:1};
var tempMissile3 = {x:700 , y:500 , go_x:-1 , go_y:-1};
var tempMissile4 = {x:0 , y:500 , go_x:1 , go_y:-1};
...
function onGameStart() {
    inervalID = setInterval(MoveMissile, 100);
}
function MoveMissile() {
    tempMissile1.x += tempMissile1.go_x * 3;
    tempMissile1.y += tempMissile1.go_y * 3;
    tempMissile2.x += tempMissile1.go_x * 3;
    tempMissile2.y += tempMissile1.go_y * 3;
    tempMissile3.x += tempMissile1.go_x * 3;
    tempMissile3.y += tempMissile1.go_y * 3;
    tempMissile4.x += tempMissile1.go_x * 3;
    tempMissile4.y += tempMissile1.go_y * 3;
    //화면 갱신
    drawScreen();
}
...
function IsColllisionWithPlayer(x, y) {
...
function MoveMissile() {
    tempMissile1.x += tempMissile1.go_x * 3;
    tempMissile1.y += tempMissile1.go_y * 3;
    if (IsColllisionWithPlayer(tempMissile1.x,tempMissile1.y)){
        GameState = GAME_STATE_OVER;
    }
    tempMissile2.x += tempMissile2.go_x * 3;
    tempMissile2.y += tempMissile2.go_y * 3;
    if (IsColllisionWithPlayer(tempMissile2.x,tempMissile2.y)){
        GameState = GAME_STATE_OVER;
    }
    tempMissile3.x += tempMissile3.go_x * 3;
    tempMissile3.y += tempMissile3.go_y * 3;
    if (IsColllisionWithPlayer(tempMissile3.x,tempMissile3.y)){
        GameState = GAME_STATE_OVER;
    }
    tempMissile4.x += tempMissile4.go_x * 3;
    tempMissile4.y += tempMissile4.go_y * 3;
    if (IsColllisionWithPlayer(tempMissile4.x,tempMissile4.y)){
        GameState = GAME_STATE_OVER;
    }
    //화면 갱신
    drawScreen();
